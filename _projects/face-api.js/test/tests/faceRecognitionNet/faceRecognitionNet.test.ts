...
describeWithBackend("faceRecognitionNet", () => {
  ...
  beforeAll(async () => {
    ...
    faceDescriptor2 = await loadJson<number[]>(
  ...
  describeWithNets(
    "quantized weights",
    { withFaceRecognitionNet: { quantized: true } },
    ({ faceRecognitionNet }) => {
  ...
  describeWithNets(
    ...
    ({ faceRecognitionNet }) => {
      it("computes face descriptors for batch of image elements", async () => {
        ...
        const results = (await faceRecognitionNet.computeFaceDescriptor(
          inputs
        )) as Float32Array[];
        ...
        results.forEach((result, batchIdx) => {
          expect(
            euclideanDistance(result, faceDescriptors[batchIdx])
          ).toBeLessThan(0.1);
        });
      });
      ...
      it("computes face descriptors for batch of mixed inputs", async () => {
        const inputs = [
          imgEl1,
          tf.browser.fromPixels(imgEl2),
          tf.browser.fromPixels(imgElRect),
        ];
  ...
  describeWithNets(
    ...
    ({ faceRecognitionNet }) => {
      describe("forwardInput", () => {
        ...
        it("single batch size 1 tf.Tensor4Ds", async () => {
          const tensor = tf.tidy(() =>
            tf.browser.fromPixels(imgEl1).expandDims()
          ) as tf.Tensor4D;

          await expectAllTensorsReleased(async () => {
            const outTensor = await faceRecognitionNet.forwardInput(
              await toNetInput(tensor)
            );
            outTensor.dispose();
          });
      ...
      describe("computeFaceDescriptor", () => {
        ...
        it("multiple image elements", async () => {
          await expectAllTensorsReleased(async () => {
            await faceRecognitionNet.computeFaceDescriptor([
              imgEl1,
              imgEl1,
              imgEl1,
            ]);
