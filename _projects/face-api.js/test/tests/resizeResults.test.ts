...
const unshiftedLandmarks = new FaceLandmarks68(
  Array(68)
    .fill(0)
    .map((_, i) => new Point(i / 100, i / 100)),
...
describe("resizeResults", () => {
  ...
  it("resizes WithFaceDetection", () => {
    ...
    const expected = detection.forSize(width, height);
    const resized = resizeResults(extendWithFaceDetection({}, detection), {
      width,
      height,
    });
  ...
  it("resizes WithFaceLandmarks", () => {
    ...
    const expectedLandmarks = unshiftedLandmarks.forSize(
      expectedRect.box.width,
      expectedRect.box.height
    );
    ...
    expect(resized.unshiftedLandmarks.imageWidth).toEqual(
      expectedRect.box.width
    );
    expect(resized.unshiftedLandmarks.imageHeight).toEqual(
      expectedRect.box.height
    );
    expectPointsClose(
      resized.unshiftedLandmarks.positions,
      expectedLandmarks.positions,
      0
    );
  });
});
