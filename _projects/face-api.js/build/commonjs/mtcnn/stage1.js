...
function extractBoundingBoxes(
  ...
  var boundingBoxes = indices.map(function (idx) {
    var cell = new tfjs_image_recognition_base_1.BoundingBox(
      Math.round((idx.y * config_1.CELL_STRIDE + 1) / scale),
      Math.round((idx.x * config_1.CELL_STRIDE + 1) / scale),
      Math.round((idx.y * config_1.CELL_STRIDE + config_1.CELL_SIZE) / scale),
      Math.round((idx.x * config_1.CELL_STRIDE + config_1.CELL_SIZE) / scale)
    );
    var score = scoresData[idx.y][idx.x];
    var regionsData = regionsTensor.arraySync();
    var region = new MtcnnBox_1.MtcnnBox(
...
function stage1(imgTensor, scales, scoreThreshold, params, stats) {
  ...
  var pnetOutputs = scales.map(function (scale) {
    return tf.tidy(function () {
      ...
      var ts = Date.now();
      var _a = PNet_1.PNet(resized, params),
        prob = _a.prob,
        regions = _a.regions;
  ...
  var boxesForScale = pnetOutputs.map(function (_a) {
    ...
    var boundingBoxes = extractBoundingBoxes(
      scoresTensor,
      regionsTensor,
      scale,
      scoreThreshold
    ...
    var ts = Date.now();
    var indices = tfjs_image_recognition_base_1.nonMaxSuppression(
  ...
  if (allBoxes.length > 0) {
    ...
    finalBoxes = indices
      ...
      .map(function (_a) {
        var cell = _a.cell,
          region = _a.region;
        return new tfjs_image_recognition_base_1.BoundingBox(
