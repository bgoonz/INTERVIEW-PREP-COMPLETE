        ...
        switch (_c.label) {
          case 0:
            ...
            imgTensor = tf.tidy(function () {
              return bgrToRgbTensor_1.bgrToRgbTensor(
                tf.expandDims(tf.browser.fromPixels(inputCanvas)).toFloat()
              );
            });
            ...
            (_b = new MtcnnOptions_1.MtcnnOptions(forwardParams)),
            ...
            scales = (
              scaleSteps ||
              pyramidDown_1.pyramidDown(minFaceSize, scaleFactor, [
                height,
                width,
              ])
            )
              .filter(function (scale) {
                var sizes = getSizesForScale_1.getSizesForScale(scale, [
                  height,
                  width,
                ]);
            ...
            stats.pyramid = scales.map(function (scale) {
              return getSizesForScale_1.getSizesForScale(scale, [
                height,
                width,
              ]);
            });
          ...
          case 3:
            ...
            results = out3.boxes.map(function (box, idx) {
              return factories_1.extendWithFaceLandmarks(
                factories_1.extendWithFaceDetection(
                  {},
                  new FaceDetection_1.FaceDetection(
                    out3.scores[idx],
                    new tfjs_image_recognition_base_1.Rect(
                      box.left / width,
                      box.top / height,
                      box.width / width,
                      box.height / height
                ...
                new FaceLandmarks5_1.FaceLandmarks5(
                  out3.points[idx].map(function (pt) {
                    return pt
                      .sub(
                        new tfjs_image_recognition_base_1.Point(
                          box.left,
                          box.top
                        )
                      )
      ...
      return tslib_1.__generator(this, function (_b) {
        switch (_b.label) {
          case 0:
            ...
            return [
              4 /*yield*/,
              tfjs_image_recognition_base_1.toNetInput(input),
            ];
          case 1:
