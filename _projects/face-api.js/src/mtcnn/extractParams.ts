...
function extractorsFactory(
  ...
  const extractConvParams = TfjsImageRecognitionBase.extractConvParamsFactory(
    extractWeights,
    paramMappings
  );
  const extractFCParams = TfjsImageRecognitionBase.extractFCParamsFactory(
    extractWeights,
    paramMappings
  );
  ...
  function extractSharedParams(
    numFilters: number[],
    mappedPrefix: string,
    isRnet: boolean = false
  ): SharedParams {
    const conv1 = extractConvParams(
      numFilters[0],
      numFilters[1],
      3,
      `${mappedPrefix}/conv1`
    );
    const prelu1_alpha = extractPReluParams(
      numFilters[1],
      `${mappedPrefix}/prelu1_alpha`
    );
    const conv2 = extractConvParams(
      numFilters[1],
      numFilters[2],
      3,
      `${mappedPrefix}/conv2`
    );
    const prelu2_alpha = extractPReluParams(
      numFilters[2],
      `${mappedPrefix}/prelu2_alpha`
    );
    const conv3 = extractConvParams(
      numFilters[2],
      numFilters[3],
      isRnet ? 2 : 3,
      `${mappedPrefix}/conv3`
    );
    const prelu3_alpha = extractPReluParams(
      numFilters[3],
