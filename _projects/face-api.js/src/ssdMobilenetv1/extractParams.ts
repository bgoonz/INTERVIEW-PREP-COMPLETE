...
function extractorsFactory(
  ...
  function extractDepthwiseConvParams(
    numChannels: number,
    mappedPrefix: string
  ): MobileNetV1.DepthwiseConvParams {
    const filters = tf.tensor4d(extractWeights(3 * 3 * numChannels), [
      3,
      3,
      numChannels,
      1,
  ...
  function extractPointwiseConvParams(
    ...
    const { filters, bias } = extractConvParams(
      channelsIn,
      channelsOut,
      filterSize,
      mappedPrefix,
  ...
  function extractConvPairParams(
    ...
    const depthwise_conv = extractDepthwiseConvParams(
      channelsIn,
      `${mappedPrefix}/depthwise_conv`
    );
    const pointwise_conv = extractPointwiseConvParams(
      channelsIn,
      channelsOut,
      1,
  ...
  function extractPredictionLayerParams(): PredictionLayerParams {
    const conv_0 = extractPointwiseConvParams(
      1024,
      256,
      1,
