...
function decodeBoxesLayer(x0: tf.Tensor2D, x1: tf.Tensor2D) {
  ...
  const div0_out = tf.div(
    tf.mul(tf.exp(tf.div(vec[2], tf.scalar(5))), sizes[0]),
    tf.scalar(2)
  );
  const add0_out = tf.add(
    tf.mul(tf.div(vec[0], tf.scalar(10)), sizes[0]),
    centers[0]
  );

  const div1_out = tf.div(
    tf.mul(tf.exp(tf.div(vec[3], tf.scalar(5))), sizes[1]),
    tf.scalar(2)
  );
  const add1_out = tf.add(
    tf.mul(tf.div(vec[1], tf.scalar(10)), sizes[1]),
    centers[1]
  );
...
export function outputLayer(
  ...
  return tf.tidy(() => {
    ...
    let boxes = decodeBoxesLayer(
      tf.reshape(
        tf.tile(params.extra_dim, [batchSize, 1, 1]),
        [-1, 4]
      ) as tf.Tensor2D,
    ...
    const scoresAndClasses = tf.sigmoid(
