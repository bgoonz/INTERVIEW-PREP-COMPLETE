...
import Flexbox from "../../../../src/blogComponents/flexbox/Flexbox"
...
In the above example we just check to see if the browser supports the `inline-size` property and if so we reset the width and use `inline-size` instead. _If you are not familiar with the @supports keyword check out [this article](/2019-12/css-supports-property)._
