...
let units_Hundreds_Position = new Array(
  "Zero",
  "One",
  "Two",
  "Three",
  ...
  */
  ...
  for (let i = 9; i >= 2; i--) {
    if (num >= i * 10) {
      ...
      outputWord += appendToPrevWord
        ? tensPosition[i - 2].toLowerCase()
        : tensPosition[i - 2];
  ...
  for (let i = 1; i < 20; i++) {
    if (num === i) {
      outputWord += appendToPrevWord
        ? units_Hundreds_Position[i].toLowerCase()
        : units_Hundreds_Position[i];
    }
  }
